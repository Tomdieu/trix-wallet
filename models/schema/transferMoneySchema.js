        ...
        custom:{
            options:async (value,{req})=>{
                if(value){
                    ...
                    if(acc_num){
                        const user_account = await Account.findOne({where:{account_number: acc_num }})
                        if(user_account){
                            if(user_account.user_id === req.user.id){
        ...
        custom:{
            options:async (value,{req})=>{
                if(value){
                    ...
                    if(x){
                        if(x<0){
                            return Promise.reject("The amount value must be a positive value")
                        }
                        else{
                            const account = await Account.findOne({where:{user_id:req.user.id}})
                            if(x > account.balance){
                                return Promise.reject("Your account balance in insufficient to make the transaction")
                            }
...
module.exports = transferMoneySchema